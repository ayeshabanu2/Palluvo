...
| **[P2] Newsletter Interactive Flow & Accessible Label** | Input lacked persistent accessible label and Join action did not trigger feedback | Connected Join action to full signup flow with `StoreContext` toast alert, confirmation message, `<label htmlFor="newsletter-email" className="sr-only">`, and `aria-label` | [`src/components/Footer.js`](src/components/Footer.js) | Clicking Join now validates email, displays instant toast confirmation, renders inline success state, and provides full accessibility. |
...
| **[P2] Newsletter signup flow & accessible label** | Wired up the newsletter form in `src/components/Footer.js` with active state, input validation, `StoreContext` toast confirmation, and inline success feedback. Added `<label htmlFor="newsletter-email" className="sr-only">Email address for newsletter</label>` and `aria-label`. |
...
- [x] **[P2] Newsletter Signup Flow:** Verified submitting the newsletter input displays a success state and triggers a confirmation toast alert.
...
| [`src/app/page.js`](src/app/page.js) | ✅ **Modified** | [P2] Scroll offset added to `#signature-models`; updated collection intro to "eight signature saree styles"; fixed hero trust badge copy; removed duplicate word "sacred" |
| [`src/components/Footer.js`](src/components/Footer.js) | ✅ **Modified** | [P2] Interactive newsletter signup flow with feedback & accessible label; assurance row: `flex-wrap gap-x-5 gap-y-2` + per-badge icons |
